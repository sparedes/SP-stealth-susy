	...
	for (Int_t i = 0;i < nMC; i++){
		...
		if(mcPID->at(i) == 22 && mcMomPID->at(i) >= 1000022 && mcMomPID->at(i) <= 1000035){				    
			...
			if(abs(DEta) <= 0.1 && abs(DPhi) <= 0.1){						
				...
				nMatchFromX++;
				if (phoPFChIso->at(j) < 0.7 && phoPFNeuIso->at(j) < (0.4+(0.4*phoEt->at(j))) && phoPFPhoIso->at(j) < (0.5+(0.005*phoEt->at(j)))) TightIsoCount++;
				if (phoPFChIso->at(j) < 1.5 && phoPFNeuIso->at(j) <( 1.0+(0.4*phoEt->at(j))) && phoPFPhoIso->at(j) < (0.7+(0.005*phoEt->at(j)))) MidIsoCount++;
				if (phoPFChIso->at(j) < 2.6 && phoPFNeuIso->at(j) < (3.5+(0.4*phoEt->at(j))) && phoPFPhoIso->at(j) < (1.3+(0.005*phoEt->at(j)))) LooseIsoCount++;
...
//       	if(mcPID->at(j) == 22 && mcMomPID->at(j) >= 1000022 && mcMomPID->at(j) <= 1000035){
